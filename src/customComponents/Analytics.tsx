...
import {ExpenseIF, App} from '../state/store';
...
const Analytics = observer(
  ({
    ...
    const groupedByWay = Object.keys(groupBy(expenses, 'expenseWay')).map(
      (key, index) => ({
        name: key,
        value: groupBy(expenses, 'expenseWay')[key].length,
        color: wayChartColors[index],
        legendFontColor: '#F0E9D2',
        legendFontSize: 10,
      }),
    );
    const groupedByType = Object.keys(groupBy(expenses, 'expenseType')).map(
      (key, index) => ({
        name: key,
        value: groupBy(expenses, 'expenseType')[key].length,
